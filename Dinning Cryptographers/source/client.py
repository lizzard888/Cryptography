...
if __name__ == "__main__":
    ...
    for v in range(id+1, clients):
        gj *= keys_num[v] % p
    gy = gi * int(gmpy.invert(gj, p)) % p
    decision = random.randint(0, 1000)
    ...
    print("Sending calculated keys.")
    while True:
        r = requests.post('https://steelforge.pl/send-second-key', cert=('certs/' + cert_name + '.crt', 'certs/' + cert_name + '.key'), verify='certs/ca/rootCA.pem', json = {'key': str(pow(gy, c, p)), 'gy': str(gy), 'gry': str(pow(gy, ran, p)), 'cr': str((c + ran) % (p -1))})
        if r.status_code == 200:
            break
        elif r.status_code == 503:
            print("Waiting for other parties...")
    ...
    result = 1
    if not r.json()['second_keys']:
        print("Error!!!! no keys")
    for x in r.json()['second_keys']:
        if x is None:
            print("Error!!!! None key")
        print("Second key is " + str(x))
        result *= int(x)
        result = result % p
    if result == 1:

...
if __name__ == "__main__":
    ...
    while True:
        ...
        time.sleep(1)
    while True:
        r = requests.post('https://steelforge.pl/get-keys', cert=('certs/' + cert_name + '.crt', 'certs/' + cert_name + '.key'), verify='certs/ca/rootCA.pem', json = {'key': str(pow(g, x, p)), 'gr': str(pow(g, ran, p)), 'xr': str(xr)})
        if r.status_code == 200:
            break
        elif r.status_code == 503:
            print("Waiting for other parties...")
